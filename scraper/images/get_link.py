...
def extrair_link_restante(linhas):
    ...
    comparacao = get_dataframe("""SELECT id, link FROM produtos WHERE id NOT IN (SELECT produto_id FROM imagens);""")
    comparacao.rename(columns={"id": "produto_id"}, inplace=True)
    start_time = time.time()

    comparacao = comparacao.head(linhas)
